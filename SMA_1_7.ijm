...
macro "SMA - Simple Muscle Architecture Analysis" {
	...
	function singleImageAnalysis() {
		...
		if (ok == 1) {
			...
			//b_1 = (Upper[0] - Upper[ex2.length-1]) / ( ex2[0] - ex2[ex2.length-1] ); //slope based on whole aponeurosis
			b_1 = (Upper[0] - Upper[x_upp.length/2-1]) / ( x_upp[0] - x_upp[x_upp.length/2-1] ); //slope based on 1/2 aponeurosis
			a_1 = Upper[0] - b_1 * x_upp[0];
			//Lower aponeurosis line (y_2 = b_2 * x + a_2)
			//b_2 = (Lower[0] - Lower[ex2.length-1]) / ( ex2[0] - ex2[ex2.length-1] ); //slope based on whole aponeurosis
			b_2 = (Lower[x_low.length/2-1] - Lower[x_low.length-1]) / ( x_low[x_low.length/2-1] - x_low[x_low.length-1] ); //slope based on 1/2 aponeurosis
